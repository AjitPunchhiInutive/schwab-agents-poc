...
from vertexai._genai.types import (
    ...
    # ReasoningEngineContextSpec wraps memory_bank_config for the AgentEngineConfig.
...
@click.option(
    "--location",
    default="us-east4",
    help="GCP region (defaults to us-east4 — must match the Agent Gateway region)",
)
@click.option(
    "--display-name",
    default="apm-lookup-agent",
    help="Display name for the agent engine",
)
@click.option(
    "--description",
    default="APM lookup agent: Cloud SQL via MCP through Agent Gateway, with Memory Bank",
    help="Description of the agent",
)
@click.option(
    "--agent-gateway",
    default=None,
    help="Full Agent Gateway resource name to route egress through "
    "(projects/PROJECT/locations/REGION/agentGateways/NAME)",
)
@click.option(
    "--source-packages",
    multiple=True,
...
def deploy_agent_engine_app(
    ...
    # --- Memory Bank integration ---
    # Wrap the memory_bank_config in a ReasoningEngineContextSpec so the
    # Agent Engine instance is created with Memory Bank enabled. This tells
    # the platform which memory topics to extract (USER_PERSONAL_INFO,
    # USER_PREFERENCES, EXPLICIT_INSTRUCTIONS) as defined in agent_engine_app.py.
    context_spec = ReasoningEngineContextSpec(
        memory_bank_config=memory_bank_config,
    )

    # --- Agent Gateway integration ---
    # Route the agent's egress (MCP/tool calls) through Agent Gateway so
    # registry, IAM x IAP, Model Armor, and semantic policies are enforced.
